...
# Control law section
	...
	# STEP 3
		...
		wait_time = 0.25
		count = 1
	# Don't allow vehicle to move if it detects unrecorded apriltag 
	else :
		l_r_tol	= 100
		forw_tol = 100
		ang_tol = 360
...
# Command send section
	send_command(command)	# Send command to vehicle using function
...
relaypin = 22
eventpin = 36
GPIO.setup(inpin,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)# Make pin input nominally low
GPIO.setup(eventpin,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)	# Set event pin to nominally low input
GPIO.setup(outpin,GPIO.OUT)			# Make pin output
GPIO.setup(relaypin,GPIO.OUT)		# Set relay pin to output

# Setting camera data event detection
GPIO.add_event_detect(eventpin,GPIO.RISING,callback=cam_data_callback,bouncetime=250)
